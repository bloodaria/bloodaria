...
async function addAccount(data) {
    ...
    // Remove any existing account before adding the new one
    let existingAccounts = accountsList.querySelectorAll('.account');
    for (let account of existingAccounts) {
        if (account.id !== 'add') {
            await new database().deleteData('accounts', account.id);
            accountsList.removeChild(account);
        }
    }
    ...
    div.innerHTML = `
        ...
        <div class="profile-infos">
           ...
           <div class="profile-uuid">COMPTE BLOODARIA</div>
        </div>
        <div class="delete-profile" id="${data.ID}">
            <i class="fa-solid fa-right-from-bracket delete-profile-icon"></i>
        </div>
    `
    
    // Hide the add button when an account is added
