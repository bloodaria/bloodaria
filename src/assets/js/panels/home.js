...
class Home {
    ...
    async init(config) {
        ...
        const notifMenuClose = document.querySelector('.notif-menu-close');
        notifBell.addEventListener('click', () => {
            if (notifMenu.classList.contains('open')) {
                notifMenu.classList.remove('open');
                notifMenu.classList.add('close');
                requestAnimationFrame(() => {
                    setTimeout(() => {
                        notifMenu.style.display = 'none';
                        notifMenu.classList.remove('close');
                    }, 300);
                });
            } else {
        ...
        notifMenuClose.addEventListener('click', () => {
            notifMenu.classList.remove('open');
            notifMenu.classList.add('close');
            requestAnimationFrame(() => {
                setTimeout(() => {
    ...
    async startGame() {
        ...
        let opt = {
            url: "https://launcher.rizyox.fr/files/?instance=Bloodaria",
    ...
    async loadNotifNews() {
        const newsList = document.querySelector('.notif-news-list');
        newsList.innerHTML = '';
        let news = await config.getNews().then(res => res).catch(err => false);
        if (news && news.length) {
            for (let News of news) {
            ...
            let block = document.createElement('div');
            block.className = 'news-block';
            block.innerHTML = `<div class=\"news-header\"><div class=\"title\">Aucune news disponible.</div></div>`;
            newsList.appendChild(block);
        }
        // Ajout du fade si scrollable
        const notifMenu = document.querySelector('.notif-menu');
        let fade = notifMenu.querySelector('.notif-menu-fade-bottom');
        if (!fade) {
            fade = document.createElement('div');
            fade.className = 'notif-menu-fade-bottom';
            notifMenu.appendChild(fade);
        }
        // Affiche le fade si scrollable
        setTimeout(() => {
            if (newsList.scrollHeight > newsList.clientHeight + 2) {
                notifMenu.classList.add('show-fade');
            } else {
                notifMenu.classList.remove('show-fade');
            }
        }, 50);
    }
}
